...
- Development patch/minor updates are grouped separately from production updates; production groups contain patch updates only, while production minors and majors remain standalone/manual. Mergify consumes Dependabot's raw `dependency-type` commit metadata such as `direct:development` / `direct:production`, so eligibility matches the environment suffix and rejects mixed dev/production batches;
